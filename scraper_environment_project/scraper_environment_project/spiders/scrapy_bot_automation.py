...
class Scrapybot(scrapy.Spider):
    ...
    def parse(self, response):
        try:
            ...
            self.clean_out_old_same_site_json(output_path)
    ...
    def clean_out_old_same_site_json(self, new_filepath):
        """
        Delete all older JSON files for the same base name (no diffing).
        """
        try:
            filename = os.path.basename(new_filepath)
            json_dir = os.path.dirname(new_filepath)

            match = re.match(r"(.+)_\d{8}_\d{6}\.json$", filename)
            if not match:
                logging.warning(f"JSON filename {filename} does not match expected pattern.")
                return

            base_name = match.group(1)

            # Find all other JSON files with same base name
            all_jsons = os.listdir(json_dir)
            old_versions = [
                f for f in all_jsons
                if f.startswith(base_name + "_") and f.endswith(".json") and f != filename
            ]

            for old_file in old_versions:
                old_path = os.path.join(json_dir, old_file)
                try:
                    os.remove(old_path)
                    logging.info(f"Removed old JSON file: {old_path}")
                except Exception as e:
                    logging.error(f"Failed to remove old JSON file {old_path}: {e}")

        except Exception as e:
            logging.error(f"Error in clean_out_old_same_site_json for {new_filepath}: {e}")
